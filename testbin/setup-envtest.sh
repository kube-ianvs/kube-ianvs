...
function setup_envtest_env {
  ...
  # Setup env vars
  IANVS_ASSETS=${IANVS_ASSETS:-""}
  if [[ -z "${IANVS_ASSETS}" ]]; then
    export IANVS_ASSETS=$1/bin
  fi
  header_text $IANVS_ASSETS
}
...
function fetch_envtest_tools {
  ...
  envtest_tools_archive_name="ianvs-tools-$goos-$goarch-latest.tar.gz"
  envtest_tools_download_url="https://github.com/kube-ianvs/ianvs-operator/releases/download/v0.0.1/$envtest_tools_archive_name"
  envtest_tools_archive_path="$tmp_root/$envtest_tools_archive_name"
  echo $envtest_tools_download_url
  echo $envtest_tools_archive_path
  ...
  echo 'tar -C "${dest_dir}"  --strip-components=1 -zvxf "$envtest_tools_archive_path"'
